function CustomAlert(){
  ...
  this.renderPlaylistInfo = function(title, dialog, urlPlay, url, ArtUrl){
    var winW = window.innerWidth;
    var winH = window.innerHeight;
    var dialogoverlay = document.getElementById('dialogOverlayForAlert');
    var dialogbox_div = document.getElementById('dialogbox-div');

    dialogoverlay.style.opacity = 0.7;
    dialogoverlay.style.background = "#212121";
    ...
    if (!mobileCheck() || !navigator.userAgentData.mobile) {
      ...
      dialogbox_div.style.backgroundImage = "url(" + ArtUrl + ")";
      dialogbox.style.border = "3px";
    } else {
    ...
    let btns = '<button id="alertboxButton" onclick="Alert.ok()">Cancel</button> &nbsp; '
    btns = btns+'<a href="' + url+ '" target="_blank"><button id="alertboxButton">View</button></a> &nbsp; '
    btns = btns+'<a href="' + urlPlay + '"><button id="alertboxButton">Play</button></a>';
